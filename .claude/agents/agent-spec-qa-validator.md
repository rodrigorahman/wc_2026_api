...
- `antipadroes_detectados[]`: cada item tem
  ...
  - `nome`: nome canônico em snake_case (ex.: `mock_driven_confidence`, `fixed_sleep_wait`, `snapshot_as_test`). Lista completa em `.claude/skills/agent-spec-testing-best-practices/references/antipadroes.md`.
...
10. **`rule_candidates_emitidos[]`**: lista de sinais para mineração offline (Camada 6.5). Não afeta veredito. Vazio é estado saudável. Vocabulário restrito a `repeated_fixture` e `repeated_assertion_shape` no escopo deste agente — outros sinais são responsabilidade de outros agentes.
...
13. **Invoke a skill `agent-spec-testing-best-practices` ANTES de produzir o JSON** — aplique a Camada 5 (Qualidade dos Testes) usando `references/antipadroes.md` como checklist. Cada antipadrão detectado vira **simultaneamente** `testing_smells.antipadroes_detectados[]` + `problemas.*` com `problema_relacionado` ligando os dois.
