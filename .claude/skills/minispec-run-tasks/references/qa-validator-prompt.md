# Prompt do Gate 1 — qa-validator (FASE 3.3)

> Referência consumida por `SKILL.md` da skill `minispec-run-tasks`.
> Leia este arquivo **antes de invocar o Gate 1** (FASE 3.3).
> Contém o prompt completo do `qa-validator` e os passos de preparação (3.1, 3.2, 3.3, 3.4, 3.5).
> Use exatamente o texto da seção "Disparar o QA" como `prompt` no `Agent({...})`.
>
> **Pré-requisito de leitura**: [`config.md`](config.md) — necessário para resolver `qa_model` (regras de escalação `qa-validator`) e os campos de `qa_summary_fields`. Leia `config.md` **antes** deste arquivo se ainda não o fez.
