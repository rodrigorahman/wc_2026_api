# Prompt do Gate 2 — staff-architecture-review-agent (FASE 4)

> Referência consumida por `SKILL.md` da skill `minispec-run-tasks`.
> Leia este arquivo **antes de invocar o Gate 2** (FASE 4.2).
> Contém o prompt completo do `staff-architecture-review-agent` e os passos de preparação (4.1, 4.2, 4.3, 4.4, 4.5, 4.6).
> Use exatamente o texto da seção "Disparar o Tech Review" como `prompt` no `Agent({...})`.
>
> **Pré-requisito de leitura**: [`config.md`](config.md) — necessário para resolver `tech_model` (regras de escalação `staff-architecture-review-agent`) e os campos de `qa_summary_fields` (sumário mínimo do QA). Leia `config.md` **antes** deste arquivo se ainda não o fez.
