---
name: minispec-run-tasks
description: Executa as tasks geradas pelo TASK PLAN do framework miniSpec. Coordenador de subagentes — orquestra, NÃO implementa diretamente. Para CADA task: delega ao executor (agent_name da stack), valida no Gate 1 (qa-validator) e Gate 2 (staff-architecture-review-agent), aplica memória lazy em rejeições e débito-controlado (críticos/altos bloqueiam; médios/baixos são anotados). User-invocable via /minispec-run-tasks.
user-invocable: true
...
2. **Se `agent_name` está ausente**:
   ...
   2. **Filtre os candidatos a executor**: remova os agentes reservados aos gates (`qa-validator`, `staff-architecture-review-agent`, `qa-test-generator`) — esses NÃO são executores.
...
| [`references/qa-validator-prompt.md`](references/qa-validator-prompt.md) | Prompt completo do Gate 1 (`qa-validator`) + passos de preparação (3.1, 3.2, 3.3) + interpretação (3.4) + loop de correção QA (3.5). | Antes de entrar na FASE 3 (Gate 1 — QA). |
| [`references/staff-review-prompt.md`](references/staff-review-prompt.md) | Prompt completo do Gate 2 (`staff-architecture-review-agent`) + passos de preparação (4.1) + interpretação (4.3) + loop de correção (4.4) + stage `git add` (4.5) + escalação ao usuário (4.6). | Antes de entrar na FASE 4 (Gate 2 — Tech Review). |
...
7. Atualize `minispec_state.yaml` (path via `minispec.state.path`):
   ...
   steps:
     execution:
       ...
       tasks_total: <N>
   ```
   Se o arquivo NÃO existir, **NÃO crie** — `minispec-generate-intent` é responsável por isso.
...
9. **Gates por task em paralelo**: despache `Agent(qa-validator)` para cada `ti` em paralelo; aguarde todos; despache `Agent(staff-architecture-review-agent)` em paralelo para os que aprovaram. Dentro de uma mesma task, QA → Tech Review permanece sequencial.
...
**Resumo do fluxo**: executor → persiste `base_sha` + sumário em memória (2.4) → leitura do `references/qa-validator-prompt.md` → `Agent(qa-validator, ...)` com `base_sha`/sumário INLINE em `instrucoes` → interpretar JSON → se rejeitado, montar memória lazy + prompt de correção e voltar ao executor; se aprovado, avançar para Gate 2.
...
## FASE 4 — Gate 2 — Tech Review (staff-architecture-review-agent)
...
**Resumo do fluxo**: QA aprovou → leitura do `references/staff-review-prompt.md` → preparar contexto + sumário mínimo do QA → `Agent(staff-architecture-review-agent, ...)` → interpretar JSON → se aprovado, `git add` real + marcar concluída; se reprovado, atualizar memória lazy + prompt de correção e voltar ao executor (revalidando AMBOS os gates após).
