...
> Este arquivo é consultado pela skill `sdd-generate-tech-spec` no momento de preencher a **Seção de Estratégia de Testes** do TECH_SPEC. A numeração varia por variante:
>
...
A seção de testes do TECH_SPEC **NÃO** deve ser preenchida pelo arquiteto. Você DEVE delegar para o subagente **`qa-test-generator`** que retorna um JSON estruturado. Depois, você converte o JSON em markdown para a seção correta.
...
1. **frente**: <web | mobile | backend>          ← NOVO. Deriva de FASE 0 da skill sdd-generate-tech-spec.
...
6. **Conformidade com `testing-best-practices`** (NOVO):
