...
# Skill: backend-contract-handoff
...
- Discovery de domínio ou design da API (use `/sdd-generate-prd`, `/sdd-generate-tech-spec`, `/minispec-generate-intent`, `/minispec-generate-scope` — esta skill consome decisões, não as toma).
- Refatoração de backend (use `/challenge-spec` ou Tech Review — esta skill descreve o estado atual, não o questiona).
