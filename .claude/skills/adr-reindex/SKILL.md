...
| Script reindex (canônico) | `adr.reindex_script` (agent-spec-adr-workflow-rules.md) → `/.claude/skills/adr-reindex/scripts/reindex.cjs` | executado UMA vez via `node {path}` |

---

# Quando usar

Esta skill é invocada exclusivamente pelo usuário via `/adr-reindex` em três cenários:

- **Recuperação** — `INDEX.md` ficou dessincronizado por edição manual de arquivos ADR.
- **Bootstrap** — após criar N ADRs em lote (ex.: durante `/adr-bootstrap`).
- **CI/CD** — validar que o `INDEX.md` commitado reflete os arquivos do repositório.

> Skills de escrita (`adr-create`, `adr-deprecate`, `adr-supersede`, `adr-bootstrap`) **já chamam o script** ao final dos seus fluxos — você não precisa rodar reindex após elas. Use esta skill apenas nos cenários acima.
...
Conteúdo fora dos marcadores (cabeçalho, instruções, linha `Ultima atualizacao`) é **preservado**. Se um dos marcadores estiver ausente, o script falha com erro claro — **não** tente recriar o `INDEX.md` daqui (é responsabilidade de `adr-create` / `adr-bootstrap`).
...
```
[adr-reindex] INDEX.md atualizado — N ADR(s) listadas.
[adr-reindex] AVISO: ... (se houver — exibir todas as linhas de aviso)
```
...
| `Diretorio ADR nao encontrado` | `{adr.dir}` inexistente | Rodar `/adr-bootstrap` ou `/adr-create` |
| `INDEX.md nao encontrado` | `{adr.index_file}` inexistente | Rodar `/adr-bootstrap` ou `/adr-create` |
...
3. Executar o script **uma única vez** via Bash: `node .claude/skills/adr-reindex/scripts/reindex.cjs`.
...
3. **NUNCA** recriar o `INDEX.md` daqui se ele faltar — encerre orientando o usuário a `/adr-bootstrap` ou `/adr-create`.
