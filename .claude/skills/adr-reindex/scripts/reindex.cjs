 ...
 *
 * Vive dentro da skill `adr-reindex` (skill canonica do reindex). Outras
 * skills do dominio ADR (adr-create, adr-deprecate, adr-supersede,
 * adr-bootstrap) referenciam este script via path resolvido pelo CLAUDE.md
 * (`adr.reindex_script`).
 *
 * Fluxo:
...
function main() {
  ...
  try {
    ...
    console.error(`[adr-reindex] ${err.message}`);
    process.exit(1);
  }
  ...
  if (!fs.existsSync(adrDir)) {
    console.error(`[adr-reindex] Diretorio ADR nao encontrado: ${adrDir}`);
    process.exit(1);
  }
  if (!fs.existsSync(indexPath)) {
    console.error(`[adr-reindex] INDEX.md nao encontrado: ${indexPath}`);
    process.exit(1);
  }
  ...
  const summary = `[adr-reindex] INDEX.md atualizado — ${entries.length} ADR(s) listadas.`;
  console.log(summary);
  for (const w of warnings) console.warn(`[adr-reindex] AVISO: ${w}`);
}
