---
name: taskcard-run-taskcard
description: Executa uma TaskCard com gates QA + Tech Review. Coordenador de subagentes — orquestra, NÃO implementa diretamente. Para a TaskCard fornecida: delega ao executor (agent_name da stack ou execução direta), valida no Gate 1 (qa-validator) e Gate 2 (staff-architecture-review-agent), aplica memória lazy em rejeições e débito-controlado (críticos/altos bloqueiam; médios/baixos são anotados). User-invocable.
user-invocable: true
...
Regras (do contrato do agente — `staff-architecture-review-agent.md` seção FLUXO DE DIFF):
