...
A seção 6 de cada task **NÃO** deve ser preenchida pelo engenheiro de tarefas. Você DEVE delegar para o subagente **`agent-spec-qa-test-generator`** que retorna um JSON estruturado. Depois, você converte o JSON em markdown no formato tabular da seção 6.
...
**Antes** de invocar qualquer `agent-spec-qa-test-generator`, verifique se o `tech_spec.md` já inclui **§14 Estratégia de Testes** com ≥ 10 CTs detalhados (input → expected → mock → rastreabilidade CA).
...
**Economia estimada**: em features onde `agent-spec-sdd-generate-tech-spec` já produz §14 completa, **elimina 70-90% das invocações QA**.
