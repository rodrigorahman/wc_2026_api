...
- **Testes**: **PENDENTE** — preenchida na FASE 2 via subagente `qa-test-generator`
...
A **seção 5 (Testes)** de cada task **NÃO** deve ser preenchida diretamente. Você DEVE delegar a geração ao subagente **`qa-test-generator`** e converter o JSON retornado em markdown estruturado tabular.
...
- **subagent_type**: `qa-test-generator` (configurado em `shared.gates.qa_test_generator`).
...
> Os gates declarados são executados pelo orquestrador `/minispec-run-tasks` na fase de execução, invocando os subagentes `qa-validator` e `staff-architecture-review-agent`. **A skill apenas declara os gates** — não os executa.
...
A Seção 5 (Testes) gerada pelo `qa-test-generator` **NÃO** é opcional:
...
9. **Testes via subagente** — **SEMPRE** delegue a Seção 5 (Testes) ao `qa-test-generator` (FASE 2). NUNCA preencha manualmente.
...
- [ ] Seção 5 (Testes) preenchida em cada task via `qa-test-generator` (delegação obrigatória)
