...
A política débito-controlado do `qa-validator` deixa passar problemas `MEDIO` e `BAIXO` (categorias `code_quality`, `naming`, `style`, `documentation`, `dead_code`, `imports`) como **débito anotado** em `qa-observations.md`. Sem essa skill, esse débito vira "cleanup futuro que nunca acontece" — exatamente o problema do post-mortem `cadastro-pratos-franquia` T8 (testes duplicados aprovados com nota 9 que ninguém limpou).

`debt-resolution` fecha o ciclo: lê os débitos acumulados, deixa o agente especialista classificar valor de correção (recomendado vs perfumaria), e deixa o usuário escolher conscientemente o que entra na fila de cleanup. Resultado: tasks prontas para rodar via `/minispec-run-tasks` sem fricção.
...
2. **agent_name** (opcional) — Nome do subagente executor da stack do projeto que vai classificar os débitos. Se omitido, descoberta interativa (igual `/minispec-run-tasks`).
...
Igual `/minispec-run-tasks`:

1. **Se `agent_name` foi informado** → usar diretamente.
2. **Se ausente**:
   - Liste subagentes em `.claude/agents/` (excluindo `qa-validator`, `staff-architecture-review-agent`, `qa-test-generator` — esses NÃO são executores).
...
- **Sem dependências entre tasks de débito** (são independentes) → marcadas como paralelizáveis em `Pode Rodar em Paralelo? = Sim`. O orquestrador `/minispec-run-tasks` aplicará seus guards de paralelização.
...
- **Testes** (seção 6): "N/A — task é cleanup; suíte existente deve continuar passando. QA executa suíte completa." (não invoca `qa-test-generator` — débitos são cleanup, não nova feature).
...
source: debt-resolution               # diferencia de execução normal
parent_version: v{N}
current_step: tasks
steps:
  intent:
...
- Comando para executar: /minispec-run-tasks docs/specs/features/{feature}/v{N+1}-debits/task_plan.md
```
...
Próximo passo:
  /minispec-run-tasks docs/specs/features/<feature>/v{N+1}-debits/task_plan.md

Tempo estimado total: ~{soma dos custos} minutos.
```

NÃO inicie `/minispec-run-tasks` automaticamente.
...
10. **NUNCA** iniciar `/minispec-run-tasks` automaticamente após gerar — apenas mostre o comando sugerido.
...
- **Tasks paralelizáveis**: débitos são por definição independentes (cada um toca seu próprio arquivo/cenário). O orquestrador `/minispec-run-tasks` honrará isso com os guards de paths disjuntos.
...
- Você quer adicionar funcionalidade nova — use `/minispec-generate-intent` (feature nova) ou `/minispec-generate-scope` (incremento), não esta skill.
