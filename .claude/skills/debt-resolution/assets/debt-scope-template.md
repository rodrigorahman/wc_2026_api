...
- [ ] `qa-observations.md` da `{{parent_version}}` ganha entrada final marcando débitos resolvidos (via FASE 4.6 da skill `/debt-resolution`).

---

## 5. Observações

- **Origem**: gerada pela skill `/debt-resolution` em {{data}}.
...
- **Não é candidato a ADR**: cleanup técnico não dispara ADR. Se durante execução algum débito revelar padrão arquitetural a registrar, sinalize ao usuário criar `/adr-create` separadamente — NÃO inclua nesta versão.
