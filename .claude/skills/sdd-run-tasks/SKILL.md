---
name: sdd-run-tasks
description: Executa as tasks geradas pelo TASK PLAN do framework SDD. Coordenador de subagentes — orquestra, NÃO implementa diretamente. Para CADA task: delega ao executor (agent_name da stack), valida no Gate 1 (qa-validator) e Gate 2 (staff-architecture-review-agent), aplica memória lazy em rejeições e débito-controlado (críticos/altos bloqueiam; médios/baixos são anotados). User-invocable.
user-invocable: true
...
5. Atualize `sdd_state.yaml` (path via `sdd.state.path`):
   ...
   steps:
     execution:
       ...
       tasks_total: <N>
   ```
   Se o arquivo NÃO existir, **NÃO crie** — `sdd-generate-prd` é responsável por isso.
...
9. **Gates por task em paralelo**: para cada `ti` do lote, despache `Agent(qa-validator)` em paralelo numa única mensagem. Aguarde TODOS. Para os que aprovaram, despache `Agent(staff-architecture-review-agent)` em paralelo. Dentro de uma mesma task, QA → Tech Review continua sequencial.
...
## Gate 2 — Tech Review (staff-architecture-review-agent)
...
```
Agent(
  subagent_type = "staff-architecture-review-agent",
