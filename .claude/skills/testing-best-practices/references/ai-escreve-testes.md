...
> Reference de `testing-best-practices`. Aplicado por `qa-test-generator` em CADA caso de teste, e por `qa-validator` ao revisar testes gerados.
>
...
No JSON do `qa-test-generator`, esses três campos vivem dentro de cada item de `casos_teste[]` (ver `qa-test-generator.md` para o schema atualizado).
...
## Resumo executivo (para uso do qa-test-generator)

Cada item de `casos_teste[]` no JSON do `qa-test-generator` ganha 4 campos novos:
...
## Resumo executivo (para uso do qa-validator)

Ao revisar testes implementados, o `qa-validator` aplica os 7 gates como **checklist de detecção**:
