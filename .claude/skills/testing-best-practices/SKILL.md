...
description: |
  ...
  (qa-test-generator) ou revisar/executar testes (qa-validator).
when_to_use: |
  - Gerar casos de teste para TECH_SPEC §14, T{N}.md §6, TaskCard §10
    (qa-test-generator).
  - Validar implementação de testes em Gate 1 do pipeline (qa-validator).
...
do_not_invoke_for: |
  - Revisão geral de código (use staff-architecture-review-agent / Gate 2).
...
- Revisão geral de código (Gate 2 / staff-architecture-review-agent).
- Debug de biblioteca de terceiros.
