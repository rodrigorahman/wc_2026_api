...
5. **Criar e salvar todas as tasks** → Para cada task:
   ...
   - Delegar Seção 6 ao `qa-test-generator` (FASE 4)
...
A seção 6 (Testes) gerada pelo `qa-test-generator` **NÃO** é opcional:
...
A **seção 6** de cada task NÃO é preenchida diretamente. Você DEVE delegar a geração ao subagente **`qa-test-generator`** e converter o JSON retornado em markdown estruturado.
...
4. Para cada chamada `qa-test-generator`: passe `qa_context.md` + arquivos relevantes da task + instruções com seções 1-5 da task parcial.
...
> Se o `sdd_state.yaml` **NÃO** existir, **não crie** — `sdd-generate-prd` é responsável por criar.
...
9. **Delegar a Seção 6 ao `qa-test-generator`** seguindo [qa-delegation-tasks.md](references/qa-delegation-tasks.md).
...
6. **NUNCA** preencha a Seção 6 manualmente — sempre delegue ao `qa-test-generator` (ou redistribua via heurística §14).
