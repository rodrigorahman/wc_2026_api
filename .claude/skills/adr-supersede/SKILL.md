...
3. **Recursos canônicos centralizados** — esta skill **não** carrega cópias próprias. Usa o template canônico de `adr-create` (`adr.template`) e o script canônico de `adr-reindex` (`adr.reindex_script`). Paths globais (`adr.dir`, `adr.index_file`) vêm de `.claude/rules/agent-spec-adr-workflow-rules.md` (rule global no system-prompt).
...
| Template ADR (canônico de `adr-create`) | `adr.template` (agent-spec-adr-workflow-rules.md) → `.claude/skills/adr-create/assets/adr-template.md` | leitura para preencher a NEW (apenas no ramo NEW_NOVA) |
...
Esta skill opera **exclusivamente** na transição `accepted | deprecated → superseded-by:NEW`. Para depreciação sem substituta, use `/adr-deprecate`.
...
Se o INDEX.md ou os marcadores estiverem ausentes, o script falha com erro claro — **não** tente recriar o INDEX a partir desta skill (responsabilidade de `adr-create`/`adr-bootstrap`).
...
b. **Validar `<old-id>`** (1º argumento, obrigatório):
   - Se ausente ou vazio → encerrar com mensagem orientando a passar o ID (ex.: `/adr-supersede 0003 0012` ou `/adr-supersede 0003`) ou rodar `/adr-list` para descobrir.
   - Normalizar para **4 dígitos** (`3` → `0003`).

c. **Localizar o arquivo da OLD** em `{adr.dir}`:
   - Listar `{adr.dir}/*.md` e encontrar `{old-id}-*.md`.
   - Se não achou → encerrar com mensagem orientando `/adr-list` (não tente "criar" a OLD).
...
> Sub-fluxo de criação **auto-contido** dentro desta skill — não delega para `adr-create`. Auto-contenção é deliberada (ver Princípios §3).
...
1. **Ler template** de `{adr.template}` (`.claude/skills/adr-create/assets/adr-template.md` — canônico, mantido por `adr-create`).
2. **Preencher**:
...
**NÃO modificar.** Convenção `superseded_keeps_applied_in: true`: o histórico de adoção fica preservado para que `/adr-review` consiga sinalizar features que ainda apontam para a OLD. **Migração para a NEW é manual e por feature.**
...
> O INDEX.md é a fonte de descoberta. Marcar `superseded-by` no frontmatter sem reindexar deixa o INDEX desatualizado e quebra `/adr-list` por status. **Não encerre a task sem rodar o reindex.**

Executar via Bash, a partir da raiz do projeto, usando o script canônico de `adr-reindex` (path em `adr.reindex_script` definido em `.claude/rules/agent-spec-adr-workflow-rules.md`):
...
Sugestao: rodar `/adr-review` apos atualizar os artefatos.
```
...
1. Resolver paths globais (`adr.dir`, `adr.index_file`, `adr.template`, `adr.reindex_script`) via **`.claude/rules/agent-spec-adr-workflow-rules.md`** (rule global no system-prompt). Esta skill **não** mantém cópias internas — usa o template canônico de `adr-create` e o script canônico de `adr-reindex`.
...
10. Rodar `node .claude/skills/adr-reindex/scripts/reindex.cjs` (script canônico, path em `adr.reindex_script`) SEMPRE ao final, **uma única vez**, depois de salvar OLD (e NEW, no ramo NEW_NOVA).
...
13. **NUNCA** sugerir/iniciar outros comandos automaticamente após o término — apenas a dica final de `/adr-review`.
14. **NUNCA** abrir ADRs além de OLD (sempre) e NEW (apenas no ramo NEW_EXISTS). Esta skill **não** varre todas as ADRs.
15. **NUNCA** recriar o INDEX.md daqui no ramo NEW_EXISTS (responsabilidade de `adr-create`/`adr-bootstrap`). No ramo NEW_NOVA, criar **apenas** o esqueleto mínimo se o INDEX não existir.
